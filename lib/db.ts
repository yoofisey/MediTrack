...
export function fetchMeds(userId: string) {
  return sb.from("medications").select("*").eq("user_id", userId).order("created_at", { ascending: false }) as unknown as Promise<{ data: Medication[] | null; error: PostgrestError | null }>;
}

export function fetchLogs(userId: string) {
  return sb.from("dose_logs").select("*, medications(name)").eq("user_id", userId).order("taken_at", { ascending: false }).limit(300) as unknown as Promise<{ data: DoseLog[] | null; error: PostgrestError | null }>;
}

export function fetchVitals(userId: string) {
  return sb.from("vitals").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(200) as unknown as Promise<{ data: Vital[] | null; error: PostgrestError | null }>;
}

export function fetchProfile(userId: string) {
  return sb.from("profiles").select("*").eq("id", userId).single() as unknown as Promise<{ data: Profile | null; error: PostgrestError | null }>;
}

export function fetchFamilyMembers(ownerId: string) {
  return sb.from("family_members").select("*").eq("owner_id", ownerId).order("created_at", { ascending: false }) as unknown as Promise<{ data: FamilyMember[] | null; error: PostgrestError | null }>;
}
...
export function fetchPendingFamilyInvites(email: string) {
  return sb.from("family_members")
    ...
    .eq("status", "pending")
    .is("member_user_id", null) as unknown as Promise<{ data: FamilyMember[] | null; error: PostgrestError | null }>;
}
