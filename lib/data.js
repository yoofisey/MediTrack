...
export async function syncVisits(userId) {
  try {
    const ownedVisits = getVisits().filter(v => !v.user_id || v.user_id === userId);
    saveVisits(ownedVisits);
    const { data } = await sb.from("visits").select("*").order("created_at", { ascending: false });
    if (!Array.isArray(data)) return;
    const serverVisits = data;
    const local = ownedVisits;
...
export function getUpcomingVisits(daysAhead = 30, userId) {
  const now = new Date();
  const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const cutoff = new Date(now.getTime() + daysAhead * 86400000);
  return getVisits().filter(v => {
    if (userId && v.user_id && v.user_id !== userId) return false;
