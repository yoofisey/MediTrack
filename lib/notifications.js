    ...
    doseTimes.forEach(doseAt => {
      ...
      if (lead > 0 && reminderAt > now) {
        ...
        doseTimers.push(setTimeout(() => {
          ...
          if (logs.filter(l => l.medication_id === med.id && l.taken_at?.startsWith(todayStr)).length >= expectedToday) return;
...
function getTodayDoseTimes(med, wakeTime) {
  ...
  const intervalMs = (med.dose_interval_hours || 24 / (med.times_per_day || 1)) * 3600000;
  const todayStart = new Date(`${new Date().toISOString().split("T")[0]}T${wakeTime || "08:00"}:00`);
  const dosesToday = med.times_per_day || 1;
...
function scheduleMissedCheck(meds, logs, todayStr) {
  ...
  missedCheckTimer = setTimeout(() => {
    ...
    meds.forEach(med => {
      ...
      const todayLogs = logs.filter(l => l.medication_id === med.id && l.taken_at?.startsWith(todayStr));
