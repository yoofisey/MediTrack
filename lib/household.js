...
export function getManagedData(memberId) {
  try {
    ...
    if (!raw) return { meds: [], logs: [] };
    const parsed = JSON.parse(raw);
    return { meds: Array.isArray(parsed.meds) ? parsed.meds : [], logs: Array.isArray(parsed.logs) ? parsed.logs : [] };
  } catch {
    return { meds: [], logs: [] };
  }
}
...
export function makeSelfMember({ user, profile, meds, logs }) {
