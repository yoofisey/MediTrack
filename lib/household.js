...
import { nextDoseDue, formatDoseTime } from "@/lib/data";
...
export function expectedDosesToday(member, now = new Date()) {
  ...
  activeMeds(member).forEach(med => {
    const due = nextDoseDue(med, member.logs || []);
    const dueMs = due ? due.getTime() : now.getTime();
    const locked = !!(due && due.getTime() > now.getTime());
    const todayStr = now.toISOString().split("T")[0];
    const logged = (member.logs || []).some(l => l.medication_id === med.id && l.taken_at?.startsWith(todayStr));
    slots.push({
      member,
      med,
      time: due ? formatDoseTime(due) : "Anytime",
      dueMs,
      logged,
      locked,
    });
...
export function nextDoseLock(member, med, now = new Date()) {
  const nextDue = nextDoseDue(med, member.logs || []);
