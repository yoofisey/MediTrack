      ...
      if (actionId === 'log') {
        ...
        plugin.schedule({
          notifications: [{
            ...
            schedule: { at: Date.now() + 300000, allowWhileIdle: true },
          }],
        }).catch(() => {});
      } else {
...
export async function scheduleDoseNotifications(meds, logs, wakeTime, leadMin, streak) {
  ...
  if (milestone) {
    const milestoneNotifications = [{
      ...
      schedule: { at: Date.now() + 5000, allowWhileIdle: true },
      sound: "beep.wav",
      ongoing: true,
      autoCancel: false,
  ...
  try {
    await plugin.schedule({
      notifications: [{
        ...
        title,
        body,
        schedule: { at: scheduleAt.getTime(), allowWhileIdle: true },
        sound: 'beep.wav',
        extra: extra || {},
      }],
    });
