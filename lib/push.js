...
function handlePushActionPerformed(notification) {
  try {
    const { data } = notification || {};
    const tag = String(data?.tag || "");
    if (tag.startsWith("mt-checkin-")) {
      window.dispatchEvent(new CustomEvent("mt-open-checkin", {}));
    } else if (tag.startsWith("mt-visit-now-")) {
      const visitId = data?.visitId || tag.slice("mt-visit-now-".length, tag.lastIndexOf("-"));
      if (visitId) window.dispatchEvent(new CustomEvent("mt-open-visit", { detail: { visitId } }));
    } else if (tag.startsWith("mt-visit-")) {
      const visitId = data?.visitId || tag.slice("mt-visit-".length, tag.lastIndexOf("-"));
      if (visitId) window.dispatchEvent(new CustomEvent("mt-open-visit", { detail: { visitId } }));
    } else if (tag.startsWith("mt-vital-")) {
      window.dispatchEvent(new CustomEvent("mt-open-vitals", { detail: { vitalType: data?.vitalType } }));
    }
  } catch {}
}
...
export async function initPushNotifications(userId) {
  ...
  const actionListener = await plugin.addListener('pushNotificationActionPerformed', handlePushActionPerformed);
...
async function displayIncomingPush(notification) {
  ...
  let actionTypeId = "DOSE_ACTION";
  const extra = { tag };
  if (tag.startsWith("mt-visit-")) {
    actionTypeId = "VISIT_ACTION";
    if (data?.visitId) extra.visitId = data.visitId;
  } else if (tag.startsWith("mt-vital-")) {
    actionTypeId = "VITAL_ACTION";
    if (data?.vitalType) extra.vitalType = data.vitalType;
  } else if (tag.startsWith("mt-daily-") || tag.startsWith("mt-checkin-") || tag.startsWith("mt-streakbreak-")) {
    actionTypeId = "MOTIVATION_ACTION";
  }
